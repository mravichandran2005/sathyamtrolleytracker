...
export default function CloseMonth({ companies, trolleyTypes, onChanged }) {
  ...
  const [myOpeningEdits, setMyOpeningEdits] = useState({})
  ...
  function openingEditValue(companyId, typeId) {
    const key = `${companyId}-${typeId}`
    return openingEdits[key] !== undefined ? openingEdits[key] : openingFor(companyId, typeId)
  }
  function myOpeningEditValue(typeId) {
    return myOpeningEdits[typeId] !== undefined ? myOpeningEdits[typeId] : myOpeningFor(typeId)
  }
      ...
      for (const c of companies) {
        for (const tt of trolleyTypes) {
          const { error } = await supabase.from('opening_stock').upsert({
            ...
            qty: openingEditValue(c.id, tt.id), set_by: profile.id,
          }, { onConflict: 'company_id,trolley_type_id,month' })
          if (error) throw error
        }
      }
      for (const tt of trolleyTypes) {
        const { error } = await supabase.from('my_company_opening_stock').upsert({
          trolley_type_id: tt.id, month: toDateStr(period),
          qty: myOpeningEditValue(tt.id), set_by: profile.id,
  ...
  async function closeMonth() {
    const count = txs.filter((t) => t.status !== 'pending').length
    const typed = window.prompt(
      `This will permanently delete ${count} transaction(s) for ${monthLabel}. Have you exported? This cannot be undone.\n\nType CLOSE to confirm.`
    )
  ...
  return (
    <div>
      <div className="card">
        <div className="row-between" style={{ flexWrap: 'wrap', gap: 12 }}>
          ...
          <ExportInventoryButton
            companies={companies} trolleyTypes={trolleyTypes} monthLabel={monthLabel}
            myOpeningFor={myOpeningFor} myMovementFor={myMovementFor} myClosingValue={myClosingValue}
            openingFor={openingFor} movementFor={movementFor} closingValue={closingValue}
          />
        </div>
      ...
      <div className="card mt-16" style={{ overflowX: 'auto' }}>
        <div className="row-between" style={{ flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h3 style={{ fontSize: 14 }}>Set opening stock for {monthLabel}</h3>
            <p style={{ fontSize: 12.5, color: 'var(--ink-dim)', marginTop: 4 }}>
              Declare this any time from a physical count — you don't need to wait for month-end. Every stock
              figure elsewhere in the app is calculated from whatever you set here.
            </p>
          </div>
          <button className="primary small" disabled={savingOpening} onClick={saveOpeningStock}>
            {savingOpening ? 'Saving…' : 'Save opening stock'}
          </button>
        </div>
        {openingSaved && <div className="banner info mt-16">Opening stock saved — figures across the app now reflect it.</div>}

        <h4 style={{ fontSize: 13, marginTop: 16, marginBottom: 8, color: 'var(--ink-dim)' }}>My Company</h4>
              ...
              <tr key={tt.id} style={{ borderTop: '1px solid var(--line)' }}>
                ...
                <td style={{ padding: 8, textAlign: 'right', width: 110 }}>
                  <input type="number" min="0" className="mono" style={{ width: 90, textAlign: 'right' }}
                    value={myOpeningEditValue(tt.id)}
                    onChange={(e) => setMyOpeningEdits((s) => ({ ...s, [tt.id]: Math.max(0, parseInt(e.target.value, 10) || 0) }))} />
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <h4 style={{ fontSize: 13, marginTop: 16, marginBottom: 8, color: 'var(--ink-dim)' }}>Partner companies</h4>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
          <tbody>
            {companies.map((c) => trolleyTypes.map((tt) => (
              <tr key={`${c.id}-${tt.id}`} style={{ borderTop: '1px solid var(--line)' }}>
                <td style={{ padding: 8 }}>{c.name}</td>
                <td style={{ padding: 8, color: 'var(--ink-dim)' }}>{tt.name}</td>
                <td style={{ padding: 8, textAlign: 'right', width: 110 }}>
                  <input type="number" min="0" className="mono" style={{ width: 90, textAlign: 'right' }}
                    value={openingEditValue(c.id, tt.id)}
                    onChange={(e) => setOpeningEdits((s) => ({ ...s, [`${c.id}-${tt.id}`]: Math.max(0, parseInt(e.target.value, 10) || 0) }))} />
                </td>
              </tr>
            )))}
          </tbody>
      ...
      <div className="card mt-16" style={{ overflowX: 'auto' }}>
        <h3 style={{ fontSize: 14, marginBottom: 12 }}>Closing projection — My Company own inventory</h3>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
          <thead>
            <tr>
      ...
      <div className="card mt-16" style={{ overflowX: 'auto' }}>
        <h3 style={{ fontSize: 14, marginBottom: 12 }}>Closing projection — Partner companies</h3>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
          <thead>
            <tr>
...
function ExportInventoryButton({ companies, trolleyTypes, monthLabel, myOpeningFor, myMovementFor, myClosingValue, openingFor, movementFor, closingValue }) {
  function exportNow() {
    const myRows = trolleyTypes.map((tt) => {
      const { dispatched, returned } = myMovementFor(tt.id)
      return {
        Type: tt.name,
        Opening: myOpeningFor(tt.id),
        'Sent out': dispatched,
        'Returned in': returned,
        Closing: myClosingValue(tt.id),
      }
    })
    ...
    companies.forEach((c) => {
      trolleyTypes.forEach((tt) => {
        ...
        partnerRows.push({
          Company: c.name,
          Type: tt.name,
          Opening: openingFor(c.id, tt.id),
          Dispatched: dispatched,
          Returned: returned,
          Closing: closingValue(c.id, tt.id),
        })
      })
    })

    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(myRows), 'My Company')
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(partnerRows), 'Partner Companies')
    const stamp = new Date().toISOString().slice(0, 10)
    XLSX.writeFile(wb, `inventory-${monthLabel.replace(' ', '-')}-${stamp}.xlsx`)
  }

  return <button className="ghost" onClick={exportNow}>Export inventory</button>
}
