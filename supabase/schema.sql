...
drop view if exists profile_names;
...
-- ----------------------------------------------------------------------------
-- PUSH SUBSCRIPTIONS — each browser/device a user has enabled notifications on
-- ----------------------------------------------------------------------------
...
-- Public-safe view: id + name only, so anyone logged in can see who created
-- or acknowledged a transaction without reading anyone's full profile.
create or replace view profile_names as select id, full_name from profiles;
